...
describe(`Article service object`, function() {
    ...
    context(`Given 'blogful_articles' table has data`, () => {
        ...
        it(`getById() resolves an article by id from 'blogful_articles' table`, () => {
            const thirdId =  3
            const thirdArticle = testArticles[thirdId-1]
            return ArticlesService.getById(db, thirdId)
                .then(actual => {
                    expect(actual).to.eql({
                        id: thirdId,
                        title: thirdArticle.title,
                        content: thirdArticle.content,
                        date_published: thirdArticle.date_published
                    })
                })
        })
    ...
    context(`Given 'blogful_articles' has no data`, () => {
        it('getAllArticles() resolves to empty array', () => {
            return ArticlesService.getAllArticles(db)
                .then(actual => {
                    expect(actual).to.eql([])
                })
